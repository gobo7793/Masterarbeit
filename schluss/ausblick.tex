...
Was ein wichtiger Punkt für Optimierungen im Testsystem darstellt ist das Starten von Anwendungen.
...
Da es hierbei jedoch bei der Ausführung eines Tests zu Problemen aufgrund nicht korrekt beendeter Anwendungen kam und daher nicht ausreichend Submitter verfügbar waren (vgl. \cref{subsec:notEnoughSubmitter}).
Eine Möglichkeit hierfür wäre, jedem Client einen eigenen Submitter bereitzustellen.
Damit einhergehend müsste jeder Client auch selbst dafür sorgen, dass Anwendungen definitiv beendet werden, wenn eine andere Anwendung gestartet werden soll (vgl. \cref{subsec:yarnClient}).
Jedoch hätte das auch den Nachteil, dass ein Client durch eine nicht zu beendende Anwendung keine weiteren Anwendungen ausführen könnte, was mit der derzeitigen Architektur des Modells möglich ist.
...
Als logische Folge erwähnt Apache selbst ebenfalls, dass Hadoop nicht \enquote{die Silberkugel [ist,] um alle Anwendungs- oder Datencenter"=Progleme zu lösen}, sondern entwickelt wurde, um einige spezifische Probleme einiger Firmen und Organisationen zu lösen \cite{HadoopIsNot}.
