...
Hierfür werden alle Komponenten wie Clients oder Anwendungen zusätzlich zur Struktur innerhalb des Modells auch direkt in entsprechenden Eigenschaften dieser Klasse gespeichert, welche zur Interaktion mit \ac{ss} entsprechend markiert sind (vgl. \cref{sec:ssharp}).
...
Die genaue Anzahl der erzeugten Instanzen kann hierbei für jede \ac{YARN}"=Komponente und auch für Clients nach Bedarf angepasst werden.
Verbunden sind die \ac{YARN}"=Komponenten und Clients mithilfe entsprechender \texttt{List<T>}"=Eigenschaften der untergeordneten Komponenten bzw. der ID und Instanz der übergeordneten Komponente.
...
Anschließend kann die Anwendung gestartet werden und ihre ID vom Client gespeichert werden.
Hierbei wird auch eine noch leere \texttt{YarnApp}"=Instanz gesucht, um diese analog zu den anderen \ac{YARN}"=Komponenten zum Speichern der Daten zu nutzen.
