...
Gespeichert und bereitgestellt werden hier Daten wie \zB die Zugangsdaten der SSH"=Verbindungen oder den genutzten \texttt{HostMode} (vgl.  \cref{subsec:implementedConnectors,subsec:hostMode}) im \ac{YARN}"=Modell und dem Treiber transparent zu machen.
Die \texttt{ModelSettings} damit zur Verwaltung des \texttt{HostMode}s zuständig und dafür verantwortlich, \zB den Pfad zum jeweils benötigten Setupscript (vgl. \cref{subsec:scripts}) dem Treiber bereit zu stellen.
