...
Die Implementierten Parser und Connectoren sind jeweils als Singleton realisiert und werden bei der Ausführung des \gls{YARN}"=Modells durch den \gls{ss}"=Simulator weitestgehend ignoriert.
Dies ist vor allem darin begründet, dass dadurch Speicher eingespart wird, der dadurch für andere \gls{YARN}"=Komponenten zur Verfügung steht.
...
Dies geschieht innerhalb des Testsystems üblicherweise entweder beim Initialisieren des \glspl{Test} (vgl. \cref{sec:implTestcases}) oder beim Initialisieren des \gls{YARN}"=Modells selbst durch die Klasse \texttt{Model}.
...
Der Parser dient dazu, die von Hadoop zurückgegebenen Daten der \gls{YARN}"=Komponenten zu konvertieren, sodass sie im Modell gespeichert werden können
...
Während bei den anderen \gls{YARN}"=Komponenten lediglich die jeweiligen IDs zurückgegeben werden, werden bei den ausführenden Nodes direkt ihre korrespondierenden Instanzen im Modell zurückgegeben.
...
Beim Abrufen und Konvertieren der Daten werden zunächst die je nach Komponente benötigten IDs von weiteren \gls{YARN}"=Komponenten ermittelt, bevor die Rohdaten durch den Connector ermittelt werden.
...
Die hierbei zurückgegebenen Daten sind im Vergleich zu den von der \gls{REST}"=API zurückgegebenen im Umfang deutlich reduziert, in den jeweiligen Übersichten der Subkomponenten auf das notwendigste beschränkt.
...
In der Liste der ausgeführten \glspl{Container} eines \gls{Attempt}s wird der Zeitstempel stattdessen im Format \texttt{ddd MMM dd HH:mm:ss zz00 yyyy} ausgegeben, was \zB dem Zeitstempel \texttt{Fri Jan 05 11:08:16 +0000 2018} entspricht.
...
Da der \gls{TLS} bei der Ausgabe der Daten zu \glspl{Attempt} und Containern zusätzliche Informationen enthält, werden hier nicht nur die Daten mithilfe des \gls{RM} bzw. der \gls{NM} der ausführenden Nodes, sondern auch mithilfe des \gls{TLS} ermittelt.
...
caption={[Konvertierung und Rückgabe eines Containers durch den RestParser]
    ...
    Hierbei muss für den hier gezeigten, einzelnen \gls{Container} zunächst die ID des übergeordneten \glspl{Attempt} ermittelt werden, bevor aus der Liste aller \gls{Container} die Daten des gesuchten Containers zurückgegeben werden können.
...
Teilweise Asynchron bedeutet hier, dass die auf dem Cluster zu startende \gls{Anwendung} nur solange synchron ausgeführt wird, bis der \gls{Anwendung} eine ID zugewiesen wurde, welche vom Connector zurückgegeben wird.
...
Die Befehlsparameter zum Starten der \glspl{Anwendung} werden von den jeweiligen Benchmarks zur Verfügung gestellt, sodass der Connector nur das Benchmark"=Script mit den bereitgestellten Befehlsparametern ausführen muss (vgl. \cref{subsec:scripts,subsec:appImplementation}).
