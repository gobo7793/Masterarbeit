...
In \cref{sec:modelArchitecture} wurde bereits aufgezeigt, dass der Treiber zur Verbindung des \gls{YARN}"=Modells mit dem realen Cluster aus den drei Komponenten Parser, Connector und der SSH"=Verbindung selbst besteht.
Der Treiber ist im \gls{YARN}"=Modell mithilfe verschiedener Interfaces zur Nutzung des Parsers und Connectors eingebunden.
Da \gls{YARN} mithilfe von Befehlen für die \gls{CLI} und einer \gls{REST}"=API zwei unterschiedliche Schnittstellen zum Auslesen der Daten der \gls{YARN}"=Komponenten für das Monitoring bereitstellt, wurden jeweils zwei entsprechende Parser und Connectoren hierfür entwickelt.
Andere Befehle wie \zB \gls{HDFS}"=Befehle können ebenfalls mithilfe des entwickelten \gls{CLI}"=Connectors ausgeführt werden, da Connectoren mithilfe von SSH"=Verbindungen mit den Cluster"=Hosts verbunden sind.

\subsection{Grundlegender Aufbau und Integration im \glsentryshort{YARN}"=Modell}
\label{subsec:driverModelIntegration}

Zur Integration des Treibers im \gls{YARN}"=Modell stellt dieser mehrere Interfaces bereit.
Dadurch sind einerseits der Treiber und das \gls{YARN}"=Modell strikt getrennt, andererseits wird es dadurch auch ermöglicht, in Zukunft andere Möglichkeiten als die hier Entwickelten zur Interaktion mit dem realen Cluster zu entwickeln und zu nutzen.

Zur Interaktion des \gls{YARN}"=Modells mit dem Treiber werden dem Modell folgende Interfaces zur Verfügung gestellt:

\begin{itemize}
    \item \texttt{IHadoopParser} für Parser
    \item \texttt{IHadoopConnector} für Connectoren
    \item Von \texttt{IParsedComponent} abgeleitete Interfaces für geparste \gls{YARN}"=Komponenten:
    \begin{itemize}
...
Das Interface \texttt{IHadoopParser} stellt hierfür entsprechende Parsing"=Methoden für die vier implementierten \gls{YARN}"=Komponenten sowie der Übersichtslisten aller einer \gls{YARN}"=Komponente untergeordneten Subkomponenten.
...
Hierfür stellen diese Interfaces entsprechende Eigenschaften bereit, um alle mithilfe der \gls{CLI} oder der \gls{REST}"=API auslesbaren Daten an das \gls{YARN}"=Modell übergeben zu können.

Das Connector"=Interface \texttt{IHadoopConnector} stellt alle zum Abrufen der Daten oder weiteren Interaktion wie das Injizieren von Komponentenfehlern oder Starten von \glspl{Anwendung} benötigten Methoden und Befehle bereit.
Hierbei wird für das Monitoring unterschieden, ob die Daten vom \gls{TLS} oder vom \gls{RM} von Hadoop abgerufen werden.
Dies ist vor allem bei der Nutzung der \gls{REST}"=API wichtig, da sich hier die Adressen und Pfade unterscheiden, während bei der Benutzung der \gls{CLI} die Befehle gleich sind .
Der \gls{TLS} wird zum Abrufen der Daten vor allem aus dem Grund genutzt, da hierbei zusätzliche Daten ermittelt werden können, die bei der reinen Nutzung der Schnittstellen des \gls{RM} nicht zurückgegeben werden würden.
...
Aus diesem Grund ist die Nutzung des \gls{TLS} zum Monitoring von \glspl{Anwendung} mithilfe des Connector"=Interfaces nicht möglich.

Die Implementierten Parser und Connectoren sind jeweils als Singleton realisiert und sind von der Serialisierung des \gls{YARN}"=Modells durch \gls{ss} ausgenommen.
Dies liegt vor allem darin begründet, dass dadurch Speicher eingespart wird, der dadurch für andere \gls{YARN}"=Komponenten zur Verfügung steht.
...
Dies geschieht innerhalb des Testsystems üblicherweise entweder beim Initialisieren des \glspl{Test} (beschrieben in \cref{sec:implTestcases}) oder beim Initialisieren des \gls{YARN}"=Modells selbst durch die Klasse \texttt{Model}.
Die Initialisierung des Modells stellt zudem den einzigen Zeitpunkt dar, bei dem im \gls{YARN}"=Modell direkt mit den implementierten Parsern und Connectoren interagiert wird, jede andere Interaktion findet stattdessen gekapselt mithilfe der Interfaces statt.
...
Der Parser dient dazu, die von Hadoop zurückgegebenen Daten der \gls{YARN}"=Komponenten einzulesen und dem Modell zu übergeben.
...
Während bei den anderen \gls{YARN}"=Komponenten nur ihre ID zurückgegeben wird, werden bei den ausführenden Nodes direkt ihre korrespondierenden Instanzen im Modell zurückgegeben.
...
Beim Abrufen und Konvertieren der Daten werden zunächst, sofern benötigt, die IDs von weiteren benötigten \gls{YARN}"=Komponenten ermittelt, bevor die Rohdaten durch den Connector ermittelt werden.
...
Da Hadoop zwei Schnittstellen in Form der \gls{CLI} und der \gls{REST}"=API bereit stellt, wurden hierfür entsprechend zwei Parser entwickelt, um die jeweiligen Daten einzulesen.

\subsubsection{Implementierung des CmdParsers}
\label{subsubsec:implCmdParser}

Der erste der beiden entwickelten Parser ist der \texttt{CmdParser} zum Monitoring mithilfe von Befehlen auf der \gls{CLI}.
...
Die hierbei zurückgegebenen Daten sind im Vergleich zu den von der \gls{REST}"=API zurückgegebenen im Umfang deutlich reduziert, in den jeweiligen Übersichten der Subkomponenten zudem auf das notwendigste beschränkt.
Im Gegenzug zur \gls{REST}"=API werden hier die Daten des \gls{RM} und \gls{TLS} kombiniert ausgegeben.
...
caption={[Implementierte \acrshort{Regex}"=Pattern im CmdParser]
    Implementierte \acrshort{Regex}"=Pattern im \texttt{CmdParser}}]
...
Diese werden von Hadoop bei der Nutzung der \gls{CLI} meist in Form des Java"=Timestamps in Millisekunden seit dem 1. Januar 1970 00:00:00 Uhr, in der Liste der ausgeführten \glspl{Container} eines \gls{Attempt}s stattdessen im Format \texttt{ddd MMM dd HH:mm:ss zz00 yyyy}.
...
Die Speicherung und Übergabe der ausführenden Nodes des \gls{AppMstr} oder der \glspl{Container} an das Modell geschieht direkt als entsprechende Node"=Instanz innerhalb des Modells.
Hierfür wird die ID bzw. die URL des Nodes genutzt, um die korrespondierende Instanz im \gls{YARN}"=Modell zu ermitteln und zu speichern.
...
Er dient dazu, die mithilfe der \gls{REST}"=API ermittelten Daten auszuwerten und an das \gls{YARN}"=Modell zu übergeben.
...
Die \gls{REST}"=API besitzt, auch im Vergleich zur \gls{CLI}"=Schnittstelle, einige Besonderheiten, auf die bei der Implementierung geachtet werden musste \cite{HadoopYarnCmds271,HadoopRmApi271,HadoopNmApi271,HadoopYarnTlServer271}:
...
\begin{itemize}
    ...
    \item \glspl{Attempt} können nur als Liste aller \glspl{Attempt} einer \gls{Anwendung} zurückgegeben werden
    ...
    \item Der \gls{TLS} enthält für \glspl{Attempt} und Anwendungs"=Container jedoch zusätzliche Daten
    \item Es werden bei Listen und Reports immer die gleichen Objekte der \gls{YARN}"=Komponenten zurückgegebenen
\end{itemize}

Da die \gls{REST}"=API die Rückgabe der Daten in zwei Formaten ermöglicht, wurde der \texttt{RestParser} aufgrund der kleineren Datenmengen und übersichtlicheren Datenformats zur Nutzung des JSON"=Formats entwickelt
Einige Beispiele für die entsprechenden Pfade der \gls{REST}"=API sowie deren Ausgaben im JSON"=Format sind in \cref{app:hadoopRestApi} zu finden.
...
Hierfür wurden neben den bestehenden Datenhaltungsklassen noch weitere Hilfsklassen entwickelt, welche das Ausgabeformat der \gls{REST}"=API nachbilden.
Mit deren Hilfe ist es möglich, alle Daten automatisch mithilfe von Json.NET aus den JSON"=Daten deserialisieren zu können.

Analog zum \texttt{CmdParser} müssen auch bei der Nutzung der \gls{REST}"=API die Zeitstempel gesondert betrachtet werden.
...
Da der \gls{TLS} bei der Ausgabe der Daten zu \glspl{Attempt} und Containern zusätzliche Informationen enthält, werden hier nicht nur die Daten mithilfe des \gls{RM} bzw. der \gls{NM} der ausführenden Nodes, sondern auch mithilfe des \gls{TLS}.
...
Aufgrund der Besonderheiten der \gls{REST}"=API im Bezug auf \glspl{Attempt} und Container, werden bei diesen beiden \gls{YARN}"=Komponenten zunächst immer die Daten aller \glspl{Attempt} einer \gls{Anwendung} bzw. aller \gls{Container} eines \glspl{Attempt} ermittelt und konvertiert.
Sollten jedoch nur die Daten jeweils eines \glspl{Attempt} bzw. Containers benötigt werden, werden diese Listen entsprechend gefiltert:
...
caption={[Konvertierung und Rückgabe der Daten durch den RestParser]
    ...
    Hierbei muss für den hier gezeigten, einzelnen \gls{Container} zunächst ID des übergeordneten \glspl{Attempt} ermittelt werden, bevor aus der Liste aller \gls{Container} die Daten des gesuchten Containers zurückgegeben werden können.
    Bei \glspl{Attempt} ist dieses Vorgehen analog.}]
...
Aufgrund dieser Besonderheiten eignet sich auch das in \cref{subsec:yarnComponentInterface} beschriebene Monitoring durch die übergeordnete Komponente bei der Nutzung der \gls{REST}"=API besser als das Selbstmonitoring.
...
\begin{itemize}
    ...
    \item Prüfen und Löschen von Daten auf dem \gls{HDFS}
...
Je nach Fähigkeiten des Connectors und Bedarf werden dadurch einzelne Verbindungen zum Monitoring, zum Injizieren und Reparieren von Komponentenfehlern oder zum Starten von \glspl{Anwendung} aufgebaut.
...
Teilweise Asynchron bedeutet hier, dass die auf dem Cluster zu startende \gls{Anwendung} nur solange synchron ausgeführt wird, bis der \gls{Anwendung} eine ID zugewiesen wurde, welche zurückgegeben wird.
Diese Funktion wird daher zum Starten der \glspl{Anwendung} durch den Client genutzt, da dieser die Anwendungs"=ID abspeichert (vgl. \cref{subsec:yarnClient}).
...
Dabei wird nicht immer direkt mit Hadoop oder den \glspl{Anwendung} interagiert, sondern immer mithilfe eines entsprechenden Setup"= oder Startscriptes (vgl. \cref{subsec:scripts}), welches vom Connector hierzu aufgerufen wird.
...
Da das Starten der \glspl{Anwendung} die einzige asynchrone Operation darstellt, die der Connector durchführen muss, werden hierfür die meisten SSH"=Verbindung aufgebaut.
Wenn eine \gls{Anwendung} gestartet werden soll, wird immer eine freie SSH"=Verbindung ausgewählt und die \gls{Anwendung} mithilfe dieser gestartet.
Die Verbindung wird dabei solange zum Starten von anderen \glspl{Anwendung} gesperrt, solange der ausführende Befehl, also die gestartete Anwendung, nicht beendet wurde (vgl. \cref{sec:sshDriver}).
...
Die Befehlsparameter zum Starten der \glspl{Anwendung} wird von den jeweiligen Benchmarks zur Verfügung gestellt, sodass der Connector nur das Benchmark"=Script mit den bereitgestellten Befehlsparametern ausführen muss (vgl. \cref{subsec:scripts,subsec:appImplementation}).

\subsubsection{Implementierung des RestConnectors}
\label{subsubsec:implRestConnector}

Der zweite implementierte Connector, der \texttt{RestConnector} dient vor allem dem Monitoring mithilfe der \gls{REST}"=API.
Aus diesem Grund lösen alle Methoden, die nicht dem Monitoring durch die \gls{REST}"=API dienen, eine entsprechende \texttt{PlatformNotSupported""Exception} aus.
...
Die Verwaltung der korrekten Adressen von \gls{RM} und \gls{TLS} erfolgt analog zum benötigten Setupscript zum \texttt{CmdConnector} durch die \texttt{ModelSettings}"=Klasse.
Ein Unterschied besteht jedoch bei den Adressen der \gls{NM} zum Monitoring der Container.
Da die Adressen der jeweiligen Nodes in der von \texttt{YarnHost} vererbten Eigenschaft \texttt{HttpUrl} gespeichert sind, werden die Adressen auch vom Connector genutzt, um mithilfe der \gls{REST}"=API die Daten der auf einem Node ausgeführten \gls{Container} zu abzurufen.
...
Hierbei wird zudem jede Kommunikation zwischen dem Treiber und dem Cluster"=Host in einer eigenen, von den \glspl{Test} unabhängigen, Logdatei gespeichert, dem SSH"=Log.
Die Verbindungsklasse ist auch dafür zuständig, dass beim teilsynchronen Starten einer \gls{Anwendung} die Anwendungs"=ID ermittelt und die \gls{Anwendung} anschließend asynchron ausgeführt wird (vgl. \cref{subsubsec:implCmdConnector,subsec:yarnClient}).
