...
Weitere zeitliche Verzögerungen während der Ausführung eines Simulations"=Schrittes sind in \autoref{subsec:simulationStep} beschrieben.
...
Die jeweiligen Instanzen der Attribute und Nodes werden für die in \autoref{subsubsec:simulationFaultActivation} beschriebene Aktivierung der dazugehörigen Komponentenfehler benötigt.
...
Die Wartezeit vor dem Monitoring ist bereits in der in\autoref{subsec:simulationBasics} beschriebenen Mindestdauer eines Schrittes enthalten.
...
Hierbei wird zunächst überprüft, ob die in \autoref{subsec:functionalRequirements} beschrieben funktionalen Anforderungen an Hadoop in Form der in \todo{constraint implementierung} implementierten Constraints für die jeweiligen Komponenten noch erfüllt werden können.
...
Zum Abschluss eines Schrittes werden die in \autoref{subsec:testRequirements} beschriebenen Behauptungen an das Testverfahren selbst validiert.
...
Die wesentlichen Ausgaben während eines Tests wurden bereits in \autoref{subsec:dataOrganisation} definiert und beschrieben.
