...
Für die Ausführung der Simulation wurden zwei grundlegende \glspl{Test} implementiert.
...
Ausgeführt werden können die \glspl{Test} mithilfe des NUnit"=Frameworks.
...
Nach Abschluss der Simulation werden immer alle noch ausgeführten \glspl{Anwendung} beendet und defekte Nodes neu gestartet, sofern nötig.
...
\begin{description}
    ...
    \item[NodeBaseCount] \hfill \\
        ...
        Benötigt wird dieser Wert, um mithilfe der \gls{REST}"=API auf die Hadoop"=Nodes zugreifen zu können, um die Daten der YARN"=Container zu ermitteln.
...
Wenn keine Eingabedaten vorab generiert werden, werden als Eingabeverzeichnisse für die \glspl{Anwendung} die Ausgabeverzeichnisse der entsprechenden Benchmarks genutzt, die die dafür benötigten Daten generieren (vgl. \cref{subsec:precreateInputData}).
...
\subsection{Ablauf eines \glsentryshort{Test} und der Testfälle}
...
Dabei wird zunächst der \gls{MARP}"=Wert aus dem Cluster ausgelesen, bevor die Benchmark\gls{Anwendung}en des Testfalls gestartet werden.
...
Die dabei vom Cluster zugewiesene ID der \gls{Anwendung} wird vom Client gespeichert, um die \gls{Anwendung} in nachfolgenden Testfällen bei Bedarf abbrechen zu können, wenn eine neue \gls{Anwendung} gestartet werden soll.
...
Dabei wird auch der \gls{MARP}"=Wert ein zweites mal ausgelesen, da er sich durch die zuvor noch aktiven bzw. neu gestarteten \glspl{Anwendung} unterschiedlich verändert haben könnte.
...
Zum Abschluss der Ausführung des \glspl{Test} werden zudem die für die Simulation als gesamtes betreffenden Constraints validiert, welche nicht im \gls{YARN}"=Modell selbst implementiert wurden (vgl. \cref{subsec:simulationBasics}).

Nach Abschluss des \glspl{Test} in Form der Simulation wird ein erneutes Monitoring des gesamten Clusters durchgeführt und der hierbei ermittelte Status als finaler Clusterstatus im Programmlog gespeichert.
...
\begin{itemize}
    ...
    \item Anzahl aller validierten Constraints und fehlerhaften Constraints, getrennt nach \gls{SuT}- und Testsystem"=Constraints
\end{itemize}
