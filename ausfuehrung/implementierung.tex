...
Hierbei wird zunächst geprüft, ob die in \todo{verweis zu Anforderungen} beschrieben funktionalen Anforderungen an Hadoop in Form der Constraints für die jeweiligen Komponenten noch erfüllt werden können.
...
Zum Abschluss eines Schrittes werden die in \todo{behauptungen im evaluationsplan} beschriebenen Behauptungen an das Testverfahren selbst geprüft.
...
Die in \todo{Verweis auf Evaluationsplan-Ausgaben} definierten Ausgaben der Eigenschaften der jeweiligen Komponenten werden im folgenden Format ausgegeben:
...
\subsection{Weitere mit der Simulation zusammenhängende Methoden}\label{sec:simulationUtilities}
