...
Der Grund für den Abbruch von 14 Tests liegt zum Großteil im in \autoref{subsubsec:simulationOracle} beschriebenen Abbruch der Simulation, wenn keine Rekonfiguration des Clusters mehr möglich ist, also bei allen Nodes des Clusters ein Komponentenfehler injiziert und dies beim Monitoring erkannt wurde.
Ein Test wurde aufgrund der zu geringen Anzahl an Submittern abgebrochen, was in \autoref{subsec:notEnoughSubmitter} genauer erläutert wird.
...
In 4 der ausgeführten Testfällen wurde jedoch kein einziger Komponentenfehler deaktiviert, weshalb die Tests der Konfigurationen 4, 5.1, 5.2 und 6 entsprechend frühzeitig abgebrochen wurden (vgl. \autoref{subsec:actDeactFaults} und \autoref{subsec:noReconf36}).
...
Nicht eingerechnet sind hier 29 nicht gestartete Anwendungen, die Gründe hierfür sind in \autoref{subsec:notStartedApps} erläutert.
...
Der Hauptgrund für die teilweise sehr hohe Anzahl an ungültigen Constraints vor allem liegt darin, dass die Constraints für fehlerhaften Anwendungen auch in nachfolgenden Testfällen innerhalb einer Ausführung einer Testkonfiguration entsprechend erkannt werden (vgl. \autoref{subsec:failedApps}).
